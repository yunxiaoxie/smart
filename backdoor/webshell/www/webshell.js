...
webshell.TerminalClass=function(id,width,height,handler) {
		...
		if(!islocked) {
			...
			r.onreadystatechange=function() {
				...
				if(r.status==200) {
					...
					cy=r.responseText.substring(45+6,48+6);
					html=r.responseText.substring(52+6);
					if(html.length>0) {
						//remove </root>
	...
	function queue(s) {
		...
		if(!islocked) {
			...
			qtimer=window.setTimeout(update,1000);
		}
	}
	...
	this.keypress = function(e) {
		...
		// the attr is a read-only property returns the numeric keyCode of the key
		...
		} else if (ev._which == 0){// need to be translated.
			switch(kc) {
	...
	this.keydown = function(e) {
		...
		if (ev) {
			...
			if (o[ev.keyCode] || ev.ctrlKey || ev.altKey) {
				...
				if (ev.stopPropagation) ev.stopPropagation();
				if (ev.preventDefault) ev.preventDefault();
				// cannot change a read-only property(which).
				ev._which=0;
