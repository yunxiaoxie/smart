...
# re-regex, glob-file path search, optparse-for command arg, 
# commands-to run linux cmd, fcntl-给文件加锁, struct-使用struct模块执行Python值和C结构体之间的转换,
# The pty module defines operations for handling the pseudo-terminal concept.
# This module provides an interface to the POSIX calls for tty I/O control.
# StringIO与cStringIO类似，都是向内存中写入文件，其操作与文件操作类似
...
class Terminal:
	...
	def reset_screen(self):
		# Screen,  i-int, c-char 0x20-' '
		# screen has w*h chars.
		...
		for c in d:
			...
			if self.vt100_keyfilter_escape:
				...
				if self.vt100_mode_lfnewline and char == 13:
					o += chr(10)
		return o
	#chr()-input num,output ascii, unichr()-input num, output unicode, ord()-input ascii/unicode, output num
	#screen is int array like:[16646220, 16646255, 16646247, 16646249, 16646254, 16646202, 16646176, 16646176]
	#the char of screen has some properties,like color, then use this num above to build frontal style, like f[0-f],b[0-f], etc.
	#w-width, h-height, y-colNum, x-cursor offset,
	def dump(self):
		dump = u""
		attr_ = -1
		...
		#y=0-30
		#traversal every char of screen
		for y in range(0, self.h):
			#count char num.
			wx = 0
			#traversal every char of line, x=0-150, self.w==width
			for x in range(0, self.w):
				#get a char from screen
				d = self.screen[y * self.w + x]
				#print('d:'+str(d))
				char = d & 0xffff #remove high(16-32), stay low(0-16).
				attr = d >> 16 #stay high
				print(str(char) + ',' +str(attr) + ',' + str(d)+','+str(x))
				# Cursor, its class is 'ff bc'
				if cy == y and cx == x and self.vt100_mode_cursor:
					attr = attr & 0xfff0 | 0x000c
				#0x0000, 0-for bg, 1-for fg, 2-for other
				# Attributes
				if attr != attr_:
					if attr_ != -1:
						dump += u'</span>'
					#like: attr254 bg14 fg15
					bg = attr & 0x000f
					fg = (attr & 0x00f0) >> 4
					# Inverse
					inv = attr & 0x0200
				...
				if char == 38:
					...
					#translate a char to charwidth, then wx add it.
					wx += self.utf8_charwidth(char)
					if wx <= self.w:
						dump += unichr(char)
			dump += "\n"# add enter for endline
		# Encode in UTF-8
...
class Multiplex:
	def __init__(self, cmd = None, env_term = None):
		...
		# Synchronize methods
        ##在同一线程内, 多次acurire不会堵塞，但必须多次release
    ...
    #You need the later when you write a program like xterm: pty.fork() in parent process returns a descriptor to control terminal of child process, 
    #so you can visually represent data from it and translate user actions into terminal input sequences.
	def proc_spawn(self, sid):
		# Session
		self.session[sid]['state'] = 'alive'
		w, h = self.session[sid]['w'], self.session[sid]['h']
		# Fork new process
        #fd is a file descriptor connected to the child’s controlling terminal
		try:
			pid, fd = pty.fork()
		except (IOError, OSError):
...
#			self.proc_finish(sid)
	...
	def proc_dump(self, sid):
		if sid not in self.session:
			return False
                content = self.session[sid]['term'].dump()
		return content
		...
		if path[0] == 'u':
			try:
				...
				if multiplex.proc_keepalive(sid, w, h):
					...
					content_data = '<?xml version="1.0" encoding="UTF-8"?><root>' + multiplex.proc_dump(sid) + '</root>';
					content_gzip = True
				else:
			...
			f = path[0] #get file like 'webshell.js'
			if f not in files:
				#if path is [''] then open first page.
				if len(f) == 0:
					f = 'webshell.html'
				else:
...
#			return BaseHTTPServer.BaseHTTPRequestHandler.log_message(self,*p)
		pass
#BaseSever提供基础的循环等待请求的处理框架。使用serve_forever启动服务，使用shutdown停止
#TCPServer在BaseServer基础上增加了一个TCP的socket连接，使用server_bind、server_activate、server_close处理TCP启停等操作
#同时增加了get_request、shutdown_request、close_request处理客户端请求。
class SecureHTTPServer(HTTPServer):
	def __init__(self, server_address, HandlerClass, cmd=None, env_term=None, ssl_enabled=False, ssl_cert=None, www_dir='www'):
